...
import { Star, Filter, ArrowRight, ListFilter, LayoutGrid, X, MapPin, Search, Palette, HandMetal, Gift, Shirt, Image, Utensils, Sparkles, Home, Scissors, Wrench, Percent } from 'lucide-react';
...
const ExplorePage = () => {
  ...
  const GigListItem = ({ gig }) => {
    ...
    return (
      <Link to={`/gigs/${gig.id}`} className="block">
        <Card className={`overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col md:flex-row card-hover w-full cursor-pointer ${gig.discount_active ? 'ring-2 ring-roman-500/40' : ''}`} dir="rtl">
          ...
          <div className="md:w-2/3 flex flex-col">
            ...
            <CardContent className="flex-grow text-right">
              ...
              <div className="text-xl font-bold text-roman-500 mb-2 flex flex-col items-start gap-1">
                {effectiveType === 'gig' && (gig.price === 0 || gig.price === '0' || gig.price === '0.00' || parseFloat(gig.price) === 0)
                  ? 'قابل للتفاوض'
                  : gig.discount_active && gig.sale_price != null ? (
                    <>
                      <span className="text-sm font-normal text-neutral-500 line-through">{Number(gig.price).toFixed(2)} جنيه</span>
                      <span className="text-xl font-bold text-roman-600">{Number(gig.sale_price).toFixed(2)} جنيه</span>
                      {gig.discount_label && (
                        <Badge className="bg-red-600 text-white text-xs w-fit">{gig.discount_label}</Badge>
                      )}
                    </>
                  )
                  : `${Number(gig.price).toFixed(2)} جنيه`}
                {effectiveType === 'product' && gig.quantity !== null && gig.quantity !== undefined && (
  ...
  return (
    <div className="container mx-auto px-4 py-8" dir="rtl">
      ...
      >
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-neutral-900 mb-2">
            {activeTab === 'sellers' ? 'تصنيفات البائعين' : activeTab === 'offers' ? 'تصفّح العروض حسب التصنيف' : 'استكشف التصنيفات'}
          </h2>
          <p className="text-roman-500">
            {activeTab === 'sellers'
              ? 'استكشف مختلف تخصصات البائعين'
              : activeTab === 'offers'
                ? 'منتجات عليها خصومات فعلية الآن مع السعر قبل وبعد العرض'
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full mb-6">
        <TabsList className={`w-full grid ${gigsEnabled ? 'grid-cols-4' : 'grid-cols-3'} mb-6 bg-neutral-100`}>
          {gigsEnabled && (
            <TabsTrigger value="gigs" className="text-lg data-[state=active]:bg-roman-500 data-[state=active]:text-white">الحرف</TabsTrigger>
          )}
          <TabsTrigger value="offers" className="text-lg data-[state=active]:bg-roman-500 data-[state=active]:text-white inline-flex items-center justify-center gap-1">
            <Percent className="h-4 w-4 hidden sm:inline shrink-0" />
            العروض
          </TabsTrigger>
          ...
          <TabsTrigger value="sellers" className="text-lg data-[state=active]:bg-roman-500 data-[state=active]:text-white">البائعين</TabsTrigger>
        </TabsList>
            ...
            <Input
              ...
              placeholder={
                activeTab === 'products' || activeTab === 'offers'
                  ? (activeTab === 'offers' ? 'بحث سريع في العروض...' : 'بحث سريع في المنتجات...')
                  : 'بحث سريع في البائعين...'
              }
        ...
        <TabsContent value="products" className="mt-0">
          <ProductExploreTabPanel
            mode="products"
            gigsEnabled={gigsEnabled}
            gigs={gigs}
            loading={loading}
            error={error}
            viewMode={viewMode}
            ...
            selectedGiftSection={selectedGiftSection}
            setSelectedGiftSection={setSelectedGiftSection}
            ...
            renderGridCard={(gig) => <GigCard gig={gig} />}
            renderListCard={(gig) => <GigListItem gig={gig} />}
          />
        </TabsContent>
        ...
        <TabsContent value="sellers" className="mt-0">
          <div className="flex flex-col md:flex-row-reverse gap-8">
            ...
            <main className="w-full md:w-3/4">
              ...
              {loading ? (
                <div className="text-center py-12">
                  <p className="text-lg text-neutral-900">جاري تحميل البائعين...</p>
                </div>
              ) : error ? (
                <div className="text-center py-12">
                  <p className="text-lg text-red-500">{error}</p>
